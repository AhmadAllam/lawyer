function displayLegalLibraryModal() {
    ...
    modalContent.innerHTML = `
        <div class="legal-library-container h-full flex">
            ...
            <div class="w-80 bg-green-50 border-l border-green-200 flex flex-col">
                <!-- رأس الشريط الجانبي -->
...
function attachLegalLibraryListeners() {
    ...
    window.addEventListener('resize', setupLegalLibraryScrollBox);
}
...
function openDownloadSite(siteNumber) {
    // مواقع التحميل
    const sites = {
        '1': 'https://www.google.com/search?q=كتب+قانونية+مجانية',
        '2': 'https://www.google.com/search?q=مكتبة+قانونية+عربية',
        '3': 'https://www.google.com/search?q=الذكاء+الاصطناعي+القانوني',
    ...
    if (!url) {
        showToast('رقم الموقع غير صحيح', 'error');
        return;
    }

    try {
        // إذا كان في Electron، استخدم الـ API
        if (window.electronAPI && window.electronAPI.openDownloadSite) {
            window.electronAPI.openDownloadSite(siteNumber);
        } else {
            // إذا كان في المتصفح، افتح في تبويب جديد
            window.open(url, '_blank');
        }
        showToast(`تم فتح الموقع`, 'success');
    } catch (error) {
        showToast('حدث خطأ في فتح الموقع', 'error');
    }
}